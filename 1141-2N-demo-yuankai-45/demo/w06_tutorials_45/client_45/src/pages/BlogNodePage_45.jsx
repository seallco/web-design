...
const BlogNodePage_45 = () => {
  ...
  // rename page-level id to avoid shadowing the blog item `id` used in the map
  const [studentId, setStudentId] = useState('213410045');
...
useEffect(()=>{
    ...
    <>
      ...
      <div className="section-title">
        <h2>blogs from Node -- { name }, {studentId}{' '}</h2>
      </div>
      <div className="blogs-center">
      {blogs_45.map((blog_45) => {
        // avoid shadowing the page-level id by renaming the blog item's id to blogId
        const { id: blogId, title, descrip, category, img } = blog_45;
        return (
        <Blog_45 
        key={blogId} 
        id={blogId} 
